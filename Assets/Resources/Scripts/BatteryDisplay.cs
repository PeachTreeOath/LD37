...
public class BatteryDisplay : MonoBehaviour {
	...
	List<GameObject> batteryIcons;
	RoombaData rd;
	Color [] cols = {Color.green, Color.yellow, Color.red, Color.black};
	...
	void Start () {
		...
		for(int i = 0; i < rd.maxBatteryLife + UpgradeManager.Instance.GetUpgradeValue(UpgradeManager.UpgradeEnum.ENERGY); i++)
		{
